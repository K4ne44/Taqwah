...
import { ALL_HADITHS, HADITH_TOPICS, HADITH_BOOKS } from "@/lib/hadith-data";
...
export default function HadithTopicPage() {
  ...
  const topicHadiths = useMemo(() => ALL_HADITHS.filter((h) => h.topics.includes(topicId)), [topicId]);

  const filteredHadiths = useMemo(() => {
    if (filterBook === "all") return topicHadiths;
    return topicHadiths.filter((h) => h.book === filterBook);
  }, [topicHadiths, filterBook]);

  const availableBooks = useMemo(() => {
    const bookIds = new Set(topicHadiths.map((h) => h.book));
    return HADITH_BOOKS.filter((b) => bookIds.has(b.id));
  }, [topicHadiths]);
  ...
  if (!topic) {
    return (
      <div className="text-center py-20">
        <p className="text-gray-400 text-lg mb-4">Topic not found</p>
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-500">{topicHadiths.length} hadiths</span>
      </div>

      {availableBooks.length > 1 && (
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
          <button onClick={() => setFilterBook("all")} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition flex-shrink-0 ${filterBook === "all" ? "bg-emerald-600 text-white" : "bg-gray-900 border border-gray-800 text-gray-400 hover:text-white hover:border-gray-700"}`}>All Books</button>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No hadiths found in this topic.</p>
          </div>
        ) : (
